...
class ContactsPage(BasePage):
    _ADD_MEMBER = (By.XPATH, '/html/body/div[1]/div/div/main/div/div/div[2]/div/div[2]/div[3]/div[10]/a[1]')
    _ADD_NAME = (By.XPATH,'//*[@id="username"]')
    _ADD_ACCOUNT_NUMBER = (By.XPATH,'//*[@id="memberAdd_acctid"]')
    _ADD_PHONE_NUMBER = (By.XPATH,'//*[@id="memberAdd_phone"]')
    _THE_LOWEST_LAYER = (By.XPATH, '/html/body/div[1]/div/div/main/div/div/div[2]/div/div[4]/div/form/div[3]')
    _CLICK_SAVE = (By.XPATH,'/html/body/div[1]/div/div/main/div/div/div[2]/div/div[4]/div/form/div[3]/a[2]')

    _CLICK_CANCEL = (By.XPATH,'//*[@class="qui_btn ww_btn js_btn_cancel"]')
    _CLICK_LEAVE = (By.XPATH, '//*[text()="离开此页"]')
    _THE_CANCEL = (By.XPATH, '//*[@class="msgDlg_right_text"]')
"""
...
def add_contacts_save(self):
    ...
    # 添加成员名称
    self.driver.find_element(*self._ADD_NAME).send_keys("Tom")
    # 添加账号
    self.driver.find_element(*self._ADD_COUNT_NUMBER).send_keys("123456")
    # 添加成员手机号
    self.driver.find_element(*self._ADD_PHONE_NUMBER).send_keys("12345678901")
    # 2. 滚动到特定元素 这一步没有下一步也可以点击最底层页面的添加按钮
    element = self.driver.find_element(*self._THE_LOWEST_LAYER)
    self.driver.execute_script("arguments[0].scrollIntoView();", element)
    time.sleep(2)
    # 点击保存
    self.driver.find_element(*self._CLICK_SAVE).click()
    # 获取弹窗文本
    texts = self.driver.switch_to.alert.text
...
def add_contacts_not_save(self):
    ...
    self.driver.find_element(*self._ADD_PHONE_NUMBER).send_keys("12345678901")
    # 点击上层取消按钮
    self.driver.find_element(*self._CLICK_CANCEL).click()

    fail_text = self.driver.find_element(*self._CLICK_LEAVE)
    if fail_text.get_attribute('innerText') == '成员的资料尚未保存，确定要离开吗？':
        self.driver.find_element(*self._THE_CANCEL).click()
    return fail_text
