...
class TestAppium(LogSet):
    ...
    def test_xueqiu(self):
        ...
        # 文本框输入 美的
        self.get_picture_source()
        with allure.step("测试步骤2：输入'美的'"):
            el2.send_keys("美的")
        # 点击搜索
        with allure.step("测试步骤3：点击输搜索"):
            self.get_picture_source()
            self.driver.find_element(by=AppiumBy.ID, value="com.xueqiu.android:id/tv_text").click()

        time.sleep(1)
        # 获取第一个搜索结果文本信息
        with allure.step("测试步骤4：获取第一个搜索结果文本信息"):
            self.get_picture_source()
...
#  cd week10_day1
#  pytest test_xueqiu_record.py --alluredir=./result --clean-alluredir
#  allure serve ./result