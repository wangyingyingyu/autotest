...
class TestWeather:
    ...
    def test_current_weather(self,city_name,expected,weather):
    ...
    @pytest.mark.parametrize("data",get_yaml(),ids=['beijing','shanghai'])
    def test_yaml(self,data):
        ...
        # 单独验证每个城市的天气信息
        expected_result = city_result[data]  # 获取对应城市的预期结果
        actual_result = self.weather.show_weather(data)  # 调用实际方法获取的结果

        assert actual_result == expected_result  # 进行断言比较

"""课堂练习：天气预测系统测试
测试要求：
Pytest fixture 及其生命周期管理：
使用 pytest.fixture 来准备测试环境，模拟不同的城市天气数据。
通过不同的作用域（如 function，module）管理和清理测试资源，确保每个测试用例有独立的环境。
在 fixture 中，进行必要的清理工作，例如删除临时数据。
conftest.py 配置：
将共享的 fixture 或钩子放在 conftest.py 文件中，便于全局共享。"""